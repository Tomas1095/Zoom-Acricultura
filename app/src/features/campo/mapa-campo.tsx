      ...
      <GestureDetector gesture={gestoCompuesto}>
        <Animated.View style={[{ position: "absolute", top: 0, left: 0, width: ancho, height: altoGrupo }, estiloAnimado]}>
          {/* El contorno del lote se dibuja con vistas comunes (un
              ...
              de cada arista) — YA NO con SVG, en ningún modo. Dos motivos.
              El viejo (por el que modo trabajo ya venía así): ni
              <Polygon>, ni <Path>, ni <Line> sueltas dibujaban bien las
              dos aristas que tocan un vértice en particular, con datos
              reales de un lote real y la rotación grande que aplica
              seguir el rumbo. El nuevo (por el que ahora también se saca
              de vista general): con el zoom horneado en la posición de
              cada punto (ver toPx, más arriba), a mucho zoom las esquinas
              del contorno terminan MUY lejos del recuadro original —
              hubo un intento de simplemente agrandar la "caja" del SVG
              para que no recorte esas esquinas (ver el historial de este
              archivo), pero la caja necesaria para cubrir el zoom máximo
              es más grande de lo que la placa gráfica puede dibujar de
              una sola vez — el contorno entero dejaba de aparecer, ni
              siquiera cortado. Las vistas comunes no tienen ese límite
              (no son una única superficie con un tamaño máximo, son
              rectángulos sueltos con posición/tamaño propios), así que no
              hay techo de zoom que las rompa.
              Costo de este cambio: se pierde el sombreado verde clarito
              de relleno que tenía el interior del lote en vista general
              (una <Path> con fill, imposible de imitar con vistas
              comunes sin mucho más código) y el contorno/recorrido pasan
              de punteados a línea sólida — igual que ya tenía modo
              trabajo (ahí nunca hubo relleno ni punteado, mismo motivo). */}
          ...
          {/* Recorrido personal — mismas vistas comunes rotadas que el
              perímetro, en los dos modos por el mismo motivo de arriba.
              En modo trabajo sigue siendo de solo lectura (se marca y
              edita siempre desde vista general). */}
          {miRutaPx.length > 1 &&
